...
class MixLoss(nn.Module):
    ...
    def forward(self, features,outputs,ce_output, labels, fea_pair_output,class_centers,epoch):
        ...
        if self.args.WCE_loss_use:
            ...
            loss += wce_loss
        if self.args.feature_similarity_use:
            similarity = torch.cosine_similarity(features[:, 0, :], features[:, 1, :], dim=1)
            similarity_loss = 1-similarity.mean()
            loss += similarity_loss
        if self.args.uncertain_use:

            evidence = F.relu(ce_output)
            alpha = evidence + 1
            uncertain = ce_output.shape[1] / torch.sum(alpha, dim=1, keepdim=True)

            uncertain_loss = 1-uncertain.mean().pow(2)
            uncertain_loss = uncertain_loss*10
